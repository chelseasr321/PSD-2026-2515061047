...
def main():
    ...
    while pilih != 7:
        ...
        print("2. Hapus Nomor Antrean")
        print("3. Tampilkan Antrean")
        print("4. Tinggi Antrean")
        print("5. Pelanggan Berikutnya")
        print("6. Pelanggan Sebelumnya")
        print("7. Keluar")
        try:
        ...
        if pilih == 1:
            ...
            print("Daftar antrean kasir: ", end="")
            bst.level_order(bst.root)
        elif pilih == 4:
            print(f"Tinggi antrean: {bst.height(bst.root)}")
        elif pilih == 5:
            try:
                x = int(input("Cari pelanggan setelah nomor: "))
                ans, found = bst.find_successor(bst.root, x)
                if found:
                    print(f"Pelanggan berikutnya adalah nomor: {ans}")
                else:
                    print("Tidak ada pelanggan berikutnya")
            except ValueError:
                print("Input tidak valid!")
        elif pilih == 6:
            try:
                x = int(input("Cari pelanggan sebelum nomor: "))
                ans, found = bst.find_predecessor(bst.root, x)
                if found:
                    print(f"Pelanggan sebelumnya adalah nomor: {ans}")
                else:
                    print("Tidak ada pelanggan sebelumnya")
            except ValueError:
                print("Input tidak valid!")
        elif pilih == 7:
